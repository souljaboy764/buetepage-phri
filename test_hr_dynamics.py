...
if __name__=='__main__':
	...
	if os.path.exists(os.path.join(MODELS_FOLDER,'hyperparams.npz')):
		...
		hri_config = hyperparams['tdm_config'].item()
	else:
	...
	for i, (x, lens) in enumerate(test_iterator):
		...
		with torch.no_grad():
			_, _, d_x1_dist = human_tdm(torch.nn.utils.rnn.pack_padded_sequence(x_p1_tdm, lens, batch_first=True, enforce_sorted=False), seq_len)
			hri_input = torch.concat([x_r2_hri, d_x1_dist.mean], dim=-1)
			z_r2hri_dist, z_r2hri_samples = hri(torch.nn.utils.rnn.pack_padded_sequence(hri_input, lens, batch_first=True, enforce_sorted=False), seq_len)
			x_r2_gen = robot_vae._output(robot_vae._decoder(z_r2hri_dist.mean))
					
		loss = F.mse_loss(x_r2_gt, x_r2_gen, reduction='none')[mask]
	...
	np.savez_compressed('hri_test.npz', x_gen=x_gen, test_data=padded_sequences.cpu().detach().numpy(), lens=lens)